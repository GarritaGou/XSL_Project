...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="//segment[@verbtenses='simple_present']">
        <font color="red"><xsl:apply-templates select="node()" /></font>
    </xsl:template>
    
    <xsl:template match="//segment[@verbtenses='present_perfect']">
        <font color="green"><xsl:apply-templates select="node()" /></font>
    </xsl:template>
	
    <xsl:template match="//segment[@verbtenses='present_continuous']">
        <font color="blue"><xsl:apply-templates select="node()" /></font>
    </xsl:template>	
	
	    <xsl:template match="//segment[@verbtenses='simple_past']">
